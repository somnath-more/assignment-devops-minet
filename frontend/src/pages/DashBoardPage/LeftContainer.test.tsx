...
describe('render left container', () => {
  it('should render the left container', () => {
    render(
      <ContextProvider>
        <LeftContainer />
      </ContextProvider>
  ...
  it('should fetch and display data when a chip is clicked', async () => {
    ...
    await waitFor(() => {
      const bitcoinChipWithText = screen.getByText('Dodge Coin');
      fireEvent.click(bitcoinChipWithText);
    });
  });
  ...
  it('should render the component', async () => {
    ...
    expect(getByText('10 coins (3 active)')).toBeInTheDocument();
    await waitFor(() => {
      expect(getByText('Dodge Coin')).toBeInTheDocument();
    });
  });
  it('should fetch and display data when a chip is clicked', async () => {
    ...
    await waitFor(() => new Promise((resolve) => setTimeout(resolve, 0)));
    await waitFor(() => {
      const bitcoinChipWithText = screen.getAllByText('BitCoin')[1];
      fireEvent.click(bitcoinChipWithText);
    });
  });
  ...
  it('should fetch and display data when a chip is clicked', async () => {
    ...
    await waitFor(() => new Promise((resolve) => setTimeout(resolve, 0)));
    await waitFor(() => {
      const bitcoinChipWithText = screen.getAllByText('Ethereum')[1];
      fireEvent.click(bitcoinChipWithText);
    });
  });
});
